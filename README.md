...
**Phase 17 shipped** — GFM table header/delimiter column-count parity vs
micromark (`looksLikeTable` only when counts match; mismatched → paragraph;
ragged body with matching header/delim still table). **Phase 16** — mixed-marker
nested lists stay one span when indented to the parent item content column
(Noto intentional golden gap #2). **Phase 15** —
...
**v0.1.14** — GFM table header/delimiter column-count parity. **v0.1.13** — mixed-marker nested lists. **v0.1.12** — setext-`---` vs hr. **v0.1.11** — adjacent defs as block starts.
...
Noto may pin `github:roobli/md#v0.1.14` when ready.

...
- [ ] Bench vs micromark on a public medium corpus (deferred to Phase 2 prep; no private vault files)

Vertical slice **shipped** on `main` (`tryNativeSplit` + micromark fallback + tests). `// replace` path in `micromark-backend.ts` / `parse.ts` remains for the full swap.
